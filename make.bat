...
set frida_version=17.9.11
set r2frida_version=6.1.4
...
cl %DEBUG% /MT /nologo /LD /Gy /D_USRDLL /D_WINDLL /DR2FRIDA_VERSION_STRING="""%r2frida_version%""" /DFRIDA_VERSION_STRING="""%frida_version%""" io_frida.c systrace.c diagnostics.c  %R2_INC% /I"%cd%" /I"%cd%\frida" "%cd%\frida\frida-core.lib" "%R2_BASE%\lib\*.lib" /link /defaultlib:setupapi.lib
if not %ERRORLEVEL%==0 (
	echo COMPILE ERROR
	exit /b 1
)
