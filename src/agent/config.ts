...
const configHelp: any[string] = {
    ...
    "systrace.enable": _configHelpSystraceEnable,
    "systrace.match": _configHelpSystraceMatch,
    "systrace.filter": _configHelpSystraceFilter,
...
const configValidator: any[string] = {
    ...
    "systrace.enable": _configValidateBoolean,
    "systrace.match": _configValidateSystraceMatcher,
    "systrace.filter": _configValidateSystraceMatcher,
...
function _configHelpSystraceEnable() {
    return `Enable the syscall tracer service and emit matches through the standard trace logger
true | false    to enable or disable syscall tracing`;
}

function _configHelpSystraceMatch() {
    return `Filter syscall names accepted by the tracer

''                  accept every syscall
open,openat,read    accept an exact comma-separated list of syscall names
/open.*/            use a regular expression when the value starts with '/'`;
}

function _configHelpSystraceFilter() {
    return `Filter syscall argument text before logging it

''                  accept every argument trace
passwd              only log syscalls whose rendered arguments contain this text
/\/tmp\/.*\.so/     use a regular expression when the value starts with '/'`;
}
...
function _configValidateSystraceMatcher(val: any): boolean {
    if (!_configValidateString(val)) {
        return false;
    }
    if (val.startsWith("/")) {
        try {
...
export function set(k: string, v: any) {
    ...
    config[k] = v;
    if (k.startsWith("systrace.")) {
        systraceCfgGen++;
    }
    if (k === "systrace.enable" || k === "systrace.match") {
        send({
            name: "action-systrace",
            stanza: {
                enabled: getBoolean("systrace.enable"),
                match: getString("systrace.match"),
            },
        });
    }
}
...
export const getSystraceCfgGen = () => systraceCfgGen;
