...
export function getPtr(p: any): NativePointer {
    ...
    if (p.startsWith("objc:")) {
        ...
        if (dot === -1) {
            ...
            if (dot === -1) {
                throw new Error(
                    "r2frida's ObjC class syntax is: objc:CLASSNAME.METHOD",
                );
            }
        }
...
export function requireFridaVersion(
    major: number,
    minor: number,
    patch: number,
) {
...
export function arrayBufferToHex(arrayBuffer: any) {
    if (
        typeof arrayBuffer !== "object" || arrayBuffer === null ||
...
export function belongsTo(modules: any[], addr: NativePointer) {
    return modules.filter((m) =>
        addr.compare(m.vmaddr) >= 0 && addr.compare(m.vmaddr.add(m.vmsize)) < 0
    );
}
