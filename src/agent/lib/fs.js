...
export function fsGet(args) {
    return _cat(args[0] || '', '*', args[1] || 0, args[2] || null);
}
export function fsCat(args) {
    return _cat(args[0] || '');
}
export function fsOpen(args) {
    return _open(args[0] || Gcwd);
}
export function chDir(args) {
...
export function transformVirtualPath(path) {
    if (fs === null) {
        fs = new FridaFS();
    }
...
export function flatify(result, vEnt, path = '') {
...
export function nsArrayMap(array, callback) {
    const result = [];
...
export function listFileDescriptors(args) {
    return listFileDescriptorsJson(args).map(([fd, name]) => {
        return fd + ' ' + name;
    }).join('\n');
}
export function listFileDescriptorsJson(args) {
    const PATH_MAX = 4096;
...
export function closeFileDescriptors(args) {
    if (args.length === 0) {
