...
import { flatify, nsArrayMap, PathTransform, VirtualEnt } from "../fs.js";
...
export const ObjCAvailable = (Process.platform === "darwin") &&
    !(Java && Java.available) && ObjC && ObjC.available && ObjC.classes &&
    typeof ObjC.classes.NSString !== "undefined";
...
export function getIOSVersion(): string {
    ...
    const versionString = processInfo.operatingSystemVersionString()
        .UTF8String().toString();
    ...
    try {
        ObjC.schedule(ObjC.mainQueue, function () {
            if (
                ...
                if (
                    retval !== undefined && retval !== null && !retval.isNull()
                ) {
                ...
                console.error(
                    `unknown method ${methodName} for objc instance at ${
                        padPointer(instancePointer)
                    }`,
                );
            }
        });
...
export function hasMainLoop(): boolean {
    ...
    const copyCurrentMode = new NativeFunction(copyCurrentModePtr, "pointer", [
        "pointer",
    ]);
    const main = getMain();
    if (main.isNull()) {
...
export function listMachoSections(baseAddr: NativePointer): any[] {
    ...
    if (machoHeader !== undefined) {
        ...
        segments
            .filter((segment) =>
                segment.name === "__TEXT" || segment.name === "__DATA"
            )
            .forEach((segment) => {
...
export class IOSPathTransform extends PathTransform {
    ...
    _fillVirtualDirs(): void {
        ...
        const appHome: string = new ObjC.Object(this.api.NSHomeDirectory())
            .toString();
        const appBundle: string = this.api.NSBundle.mainBundle().bundlePath()
            .toString();
        ...
        if (groupNames.length > 0) {
            ...
            for (const groupName of groupNames) {
                const groupUrl = fileManager
                    .containerURLForSecurityApplicationGroupIdentifier_(
                        groupName,
                    );
                if (groupUrl !== null) {
                    appGroups.addSub(
                        new VirtualEnt(groupName, groupUrl.path().toString()),
                    );
                }
            }
        }
    ...
    _getAppGroupNames(): string[] {
        ...
        const key = this.api.NSString.stringWithString_(
        ...
        if (this._api === null) {
            this._api = {
                ...
                SecTaskCreateFromSelf: new NativeFunction(
                    getGlobalExportByName("SecTaskCreateFromSelf")!,
                    "pointer",
                    ["pointer"],
                ),
                SecTaskCopyValueForEntitlement: new NativeFunction(
                    getGlobalExportByName("SecTaskCopyValueForEntitlement")!,
                    "pointer",
                    ["pointer", "pointer", "pointer"],
                ),
                CFRelease: new NativeFunction(
                    getGlobalExportByName("CFRelease")!,
                    "void",
                    ["pointer"],
                ),
            };
