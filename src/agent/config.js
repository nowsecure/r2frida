...
export function asR2Script () {
  return Object.keys(config)
    .map(k => ':e ' + k + '=' + config[k])
    .join('\n');
}
export function getString (k) {
  const ck = config[k];
...
export function evalConfigR2 (args) {
  return asR2Script();
}
export function evalConfig (args) {
  if (args.length === 0) {
    return asR2Script();
  }
...
export function evalConfigSearch (args) {
