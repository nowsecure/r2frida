...
static char *__system_continuation(RIO *io, RIODesc *fd, const char *command) {
	...
	if (!strcmp (command, "help") || !strcmp (command, "h") || !strcmp (command, "?")) {
		...
		":di[0,1,-1] [addr]          Intercepts and replace return value of address without calling the function\n"
		":dif[0,1,-1] [addr]         Intercepts return value of address after calling the function\n"
	...
	if (!strcmp (command, "")) {
		...
		io->cb_printf ("  z  = show pointer to UTF-8 string\n");
