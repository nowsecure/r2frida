...
static int __system(RIO *io, RIODesc *fd, const char *command) {
	...
	if (!strcmp (command, "help") || !strcmp (command, "h") || !strcmp (command, "?")) {
		io->cb_printf ("r2frida commands available via =!\n"
			...
			"ds <size>                  Allocate <size> bytes on the heap, address is returned\n"
			"dsa <string>               Allocate a string inited with <string> on the heap\n"
			"dsd <addr> <size>          Allocate <size> bytes on the heap, copy contents from <addr>\n"
			"dsl                        List live allocations\n"
			"ds- (<addr>...)            Kill the allocations at <addr> (or all of them without param)\n"
